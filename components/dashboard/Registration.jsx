...
export default function Registration({ onDone }) {
  ...
  const fetchRegistrations = async () => {
    try {
      const res = await fetch(`${BASE_URL}/patient-registrations/${username}/`);
  ...
  return null;
    ...
    try {
      const res = await fetch(`${BASE_URL}/submit-registration/`, {
        method: "POST",
      ...
      if (res.ok) {
        setSavedList((prev) => [
          ...prev,
          { form, answers: finalAnswers, queue_number: data.queue_number },
        ]);
  ...
  const handleEdit = (index) => {
    ...
    setSelectedIndex(i);
    setEditing(true);
    setPhase("questions"); // ← i-change ni gikan sa "form" ngadto sa "questions"
    setCurrentQ(0);
  };
      ...
      <View style={{ padding: 20 }}>
        ...
        {savedList.length === 0 ? (
          ...
          >
            ...
            >
              Tap{" "}
              <Text style={{ fontWeight: "700", color: C.teal }}>＋ New</Text>{" "}
