...
const ReminderCard = ({ icon, title, text, borderColor, dark }) => (
  <View style={[styles.card, { borderLeftColor: borderColor, backgroundColor: dark ? '#242b2a' : '#fff' }]}>
    <Text style={[styles.icon, { fontSize: 28 }]}>{icon}</Text>
    <View style={styles.content}>
      <Text style={[styles.cardTitle, { color: dark ? '#e8f0ef' : '#333', fontSize: 16 }]}>{title}</Text>
      <Text style={[styles.cardText, { color: dark ? '#7aada8' : '#666', fontSize: 14, lineHeight: 20 }]}>{text}</Text>
    </View>
  </View>
);
...
export default function Reminders() {
  ...
  return (
    <View style={styles.container}>
      <Text style={[styles.title, { color: dark ? '#e8f0ef' : '#333', fontSize: 17 }]}>✓ What to Bring</Text>
      ...
      <View style={styles.proTipSection}>
        <ReminderCard icon="💡" title="Pro Tip" text="Bring a foldable chair, handheld mini fan or hand fan, water, and snacks. Arrive early, especially for your first dose." borderColor="#fbc02d" dark={dark} />
      </View>
