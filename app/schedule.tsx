...
export default function PatientScheduleScreen() {
  ...
  const { darkMode, username, userId } = useUser();   // ← userId from context
  const authFetch = useAuthFetch();                   // ← hook called correctly
  const dark = darkMode;
  ...
  const fetchSchedule = async () => {
    try {
      ...
      // Step 2: Fetch dose schedule for this patient
      const data: any[] = await authFetch(`/dose-schedules/patient/${patientId}/`);

      if (Array.isArray(data) && data.length > 0) {
        const mapped: Dose[] = data.map((item: any) => ({
          id:        item.id,
          name:      item.dose_name,
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: C.contentBg }}>
          ...
          <View style={styles.contentPadding}>
            ...
            <View style={[styles.progressCard, { backgroundColor: C.card, borderColor: C.border }]}>
              <View style={styles.progressHeader}>
                ...
                <View style={[styles.progressBadge, { backgroundColor: allRequiredDone ? "#e8f5e9" : dark ? "#1e2d3d" : "#e3f2fd" }]}>
                  <Text style={[styles.progressBadgeText, { color: allRequiredDone ? C.green : C.blue }]}>
              ...
              <View style={[styles.progressBarBg, { backgroundColor: C.progressBg }]}>
                <View style={[styles.progressBarFill, { width: `${progressPercent}%` as any, backgroundColor: allRequiredDone ? C.green : C.teal }]} />
              </View>
              ...
              <View style={styles.stepsRow}>
                {doses.map((dose, index) => (
                  <View key={dose.id} style={styles.stepItem}>
                    {index > 0 && (
                      <View style={[styles.stepLine, { backgroundColor: doses[index - 1].completed ? C.teal : C.progressBg }]} />
                    )}
                    <View style={[styles.stepCircle, {
                      ...
                      {dose.completed
                        ...
                        : <Text style={[styles.stepNum, { color: C.sub }]}>{index + 1}</Text>
                      }
                    </View>
                    <Text style={[styles.stepLabel, { color: dose.completed ? C.teal : C.sub }, dose.optional && { fontStyle: "italic" }]}>
            ...
            {doses.map((dose) => (
              <View
                ...
                style={[
                  ...
                  dose.optional && !dose.completed && { borderLeftColor: "#ff9800" },
                ]}
              >
                <View style={[
                ...
                <View style={styles.doseInfo}>
                  ...
                  <Text style={[styles.doseDate, { color: C.sub }]}>📅 {formatDate(dose.date)}</Text>
                  {dose.completed && <Text style={[styles.congratsText, { color: C.green }]}>✓ Done!</Text>}
                </View>
              </View>
            ))}
              ...
              <View style={[styles.allDoneBanner, { backgroundColor: dark ? "#1a3320" : "#e8f5e9", borderColor: C.green }]}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={[styles.allDoneSub, { color: C.sub }]}>
                    You are fully protected. Consider the booster for extra immunity.
                  </Text>
                </View>
              </View>
            )}
            ...
            <View style={[styles.infoBox, { backgroundColor: C.inputBg, borderColor: C.border }]}>
              ...
              <Text style={[styles.infoText, { color: C.sub }]}>• Your schedule is managed by the health center</Text>
...
const styles = StyleSheet.create({
  root:               { flex: 1 },
  header:             { paddingVertical: 14, paddingHorizontal: 16, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  backBtn:            { backgroundColor: "rgba(255,255,255,0.15)", borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6 },
  backText:           { color: "#fff", fontWeight: "700", fontSize: 14 },
  headerTitle:        { color: "#fff", fontWeight: "800", fontSize: 16 },
  container:          { flex: 1 },
  contentPadding:     { padding: 16, paddingTop: 12 },
  progressCard:       { borderRadius: 14, borderWidth: 1, padding: 16, marginBottom: 16, elevation: 3 },
  ...
  doseCard:           { borderLeftWidth: 4, padding: 14, borderRadius: 8, marginBottom: 12, flexDirection: "row", alignItems: "center", elevation: 1 },
  checkboxInner:      { width: 24, height: 24, borderRadius: 6, borderWidth: 2, justifyContent: "center", alignItems: "center", marginRight: 12 },
  ...
  optionalTag:        { backgroundColor: "#fff3e0", borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2 },
