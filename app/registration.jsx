...
// ── Field must be defined OUTSIDE the screen component so it never remounts on re-render
...
export default function RegistrationScreen() {
  ...
  // ── Core phase state ─────────────────────────────────────────────────────────
  // phases: 'list' | 'questions' | 'form' | 'view'
  ...
  // ── Multi-registration list ──────────────────────────────────────────────────
  const [savedList, setSavedList]       = useState([]);   // array of { form, answers }
  const [selectedIndex, setSelectedIndex] = useState(null); // which registration is being viewed/edited
  ...
  const triggerLimitAlert = () => {
    ...
    Animated.sequence([
      Animated.timing(limitAlertOpacity, { toValue: 1, duration: 250, useNativeDriver: true }),
    ]).start();
    setShowLimitAlert(true);
    limitAlertTimer.current = setTimeout(() => {
      Animated.timing(limitAlertOpacity, { toValue: 0, duration: 400, useNativeDriver: true }).start(() => {
  ...
  const fetchRegions = async () => {
    try {
      setAddrLoading('region');
      const res = await fetch(`${PSGC}/regions`);
      const data = await res.json();
      setAddrRegions(data.sort((a, b) => a.name.localeCompare(b.name)));
    } catch { Alert.alert('Network Error', 'Could not load regions. Please check your connection.'); }
    finally { setAddrLoading(''); }
  };

  const fetchProvinces = async (regionCode) => {
    try {
      setAddrLoading('province');
      setAddrProvinces([]); setAddrCities([]); setAddrBarangays([]);
      setAddrProvince(null); setAddrCity(null); setAddrBarangay(null);
      const res = await fetch(`${PSGC}/regions/${regionCode}/provinces`);
      const data = await res.json();
      setAddrProvinces(data.sort((a, b) => a.name.localeCompare(b.name)));
      setAddrOpen('province');
    } catch { Alert.alert('Network Error', 'Could not load provinces.'); }
    finally { setAddrLoading(''); }
  };
  ...
  const fetchCities = async (provinceCode) => {
    try {
      setAddrLoading('city');
      setAddrCities([]); setAddrBarangays([]);
      setAddrCity(null); setAddrBarangay(null);
      const [cRes, mRes] = await Promise.all([
        fetch(`${PSGC}/provinces/${provinceCode}/cities`),
        fetch(`${PSGC}/provinces/${provinceCode}/municipalities`),
      ]);
      const cities = await cRes.json();
      const munis  = await mRes.json();
      const combined = [...(Array.isArray(cities) ? cities : []), ...(Array.isArray(munis) ? munis : [])];
      setAddrCities(combined.sort((a, b) => a.name.localeCompare(b.name)));
      setAddrOpen('city');
    } catch { Alert.alert('Network Error', 'Could not load cities.'); }
    finally { setAddrLoading(''); }
  };

  const fetchBarangays = async (cityCode) => {
    try {
      setAddrLoading('barangay');
      setAddrBarangays([]); setAddrBarangay(null);
      const res = await fetch(`${PSGC}/cities-municipalities/${cityCode}/barangays`);
      const data = await res.json();
      setAddrBarangays(data.sort((a, b) => a.name.localeCompare(b.name)));
      setAddrOpen('barangay');
    } catch { Alert.alert('Network Error', 'Could not load barangays.'); }
    finally { setAddrLoading(''); }
  };

  const openAddrPicker = () => {
    setShowAddrPicker(true);
    setAddrOpen('region');
    if (addrRegions.length === 0) fetchRegions();
  };

  const saveAddress = () => {
    if (!addrRegion || !addrProvince || !addrCity || !addrBarangay) {
      Alert.alert('Incomplete', 'Please select all address fields.');
      return;
    }
    const full = `${addrBarangay.name}, ${addrCity.name}, ${addrProvince.name}, ${addrRegion.name}, Philippines`;
  ...
  const clearAddress = () => {
    animate();
    setForm(p => ({ ...p, address: '' }));
    setAddrRegion(null); setAddrProvince(null); setAddrCity(null); setAddrBarangay(null);
  };
  ...
  const confirmBirthdate = () => {
    if (!bdDay) return;
    const formatted = `${MONTHS[bdMonth]} ${String(bdDay).padStart(2, '0')}, ${bdYear}`;
    const age = calcAge(bdYear, bdMonth, bdDay);
    setForm(p => ({ ...p, birthdate: formatted, age }));
  ...
  const resetWorkingState = () => {
    ...
    setAnswers({});
    setOthersText('');
    setForm({ fullName: '', age: '', birthdate: '', address: '', contact: '' });
    setEditing(false);
  ...
  const getQuestion = (q) => {
    if (q.id === 3) return { ...q, label: getQ3Label() };
    if (q.id === 6) return { ...q, label: getQ6Label() };
    return q;
  };
  ...
  const handleSave = () => {
    ...
    if (!fullName.trim() || !age.trim() || !birthdate.trim() || !address.trim() || !contact.trim()) {
    ...
    const newEntry = { form, answers: finalAnswers };
    setSavedList(prev => [...prev, newEntry]);
    setSelectedIndex(savedList.length); // will point to the new entry
    setPhase('view');
  };

  const handleDelete = (index) => {
    const targetIndex = index !== undefined ? index : selectedIndex;
    Alert.alert('Delete Registration', 'Are you sure you want to delete this registration?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => {
        animate();
        setSavedList(prev => prev.filter((_, i) => i !== targetIndex));
        resetWorkingState();
        setPhase('list');
      }},
    ]);
  ...
  const handleEdit = (index) => {
    const targetIndex = index !== undefined ? index : selectedIndex;
    const entry = savedList[targetIndex];
    if (!entry) return;
    animate();
    setForm(entry.form);
  ...
  const handleSaveEdit = () => {
    ...
    setSavedList(prev => prev.map((entry, i) => i === selectedIndex ? updated : entry));
    setEditing(false);
    setPhase('view');
  };

  const startNewRegistration = () => {
    if (savedList.length >= MAX_REGISTRATIONS) {
      triggerLimitAlert();
      return;
    }
    animate();
  ...
  const confirmDate = () => {
    if (!dpDay) return;
    const formatted = `${MONTHS[dpMonth]} ${String(dpDay).padStart(2, '0')}, ${dpYear}`;
    setAnswers(prev => ({ ...prev, incidentDate: formatted }));
  ...
  const clearDate = () => {
    animate();
    setAnswers(prev => ({ ...prev, incidentDate: null }));
  };
      ...
      <View style={{ width: 200, height: 200, backgroundColor: '#fff', padding: 12, borderRadius: 8, borderWidth: 2, borderColor: C.teal, justifyContent: 'center', alignItems: 'center' }}>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: 176, height: 176 }}>
          {Array.from({ length: 22 * 22 }).map((_, i) => {
            ...
            return (
              <View key={i} style={{ width: 8, height: 8, backgroundColor: (isCorner || hash === 0) ? '#1a1a1a' : '#ffffff' }} />
            );
          })}
        </View>
      </View>
      <Text style={{ color: C.sub, fontSize: 10, marginTop: 8, textAlign: 'center' }}>
        Scan this QR code at the front desk for verification
      </Text>
    </View>
  );

  // ── RENDER: Registration List (main landing) ─────────────────────────────────
  const renderList = () => {
    ...
    return (
      <View style={{ flex: 1, padding: 20 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <View>
            ...
            <Text style={{ fontSize: 12, color: C.sub, marginTop: 2 }}>
              {savedList.length} / {MAX_REGISTRATIONS} used
            </Text>
          </View>

          {/* +New button */}
          <TouchableOpacity
            onPress={startNewRegistration}
            activeOpacity={atLimit ? 0.6 : 0.85}
            style={{
              flexDirection: 'row', alignItems: 'center', gap: 6,
              backgroundColor: atLimit ? (dark ? '#2e3837' : '#e0e0e0') : C.teal,
              paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12,
              elevation: atLimit ? 0 : 3,
            }}
          >
        ...
        {showLimitAlert && (
          <Animated.View style={{
            opacity: limitAlertOpacity,
            backgroundColor: dark ? '#3b1f1f' : '#fff3e0',
            borderRadius: 12,
            padding: 13,
            borderWidth: 1.5,
            ...
            <Text style={{ color: C.orange, fontWeight: '700', fontSize: 13, flex: 1 }}>
              You've reached the maximum registration limit!
            </Text>
          </Animated.View>
        )}
        ...
        {savedList.length === 0 ? (
          <View style={{ backgroundColor: C.card, borderRadius: 20, padding: 32, borderWidth: 1, borderColor: C.border, alignItems: 'center', marginTop: 8 }}>
            ...
            <Text style={{ fontSize: 16, fontWeight: '800', color: C.text, textAlign: 'center', marginBottom: 8 }}>
              No Registrations Yet
            </Text>
            <Text style={{ fontSize: 13, color: C.sub, textAlign: 'center', lineHeight: 20, marginBottom: 20 }}>
              Tap <Text style={{ fontWeight: '700', color: C.teal }}>＋ New</Text> to register a patient for their first dose at ABTC-CHO.
            </Text>
              ...
              return (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ fontSize: 12, color: C.sub, marginTop: 2 }}>
                      {animalEmoji} {entry.answers.animalType}  {injuryEmoji} {entry.answers.injuryType}
                    </Text>
                    <Text style={{ fontSize: 11, color: C.sub, marginTop: 2 }} numberOfLines={1}>
                      📅 {entry.answers.incidentDate}
                    </Text>
                  </View>
            ...
            {savedList.length < MAX_REGISTRATIONS && (
              <View style={{ alignItems: 'center', paddingVertical: 8 }}>
                <Text style={{ color: C.sub, fontSize: 12 }}>
                  {MAX_REGISTRATIONS - savedList.length} slot{MAX_REGISTRATIONS - savedList.length !== 1 ? 's' : ''} remaining
                </Text>
              </View>
            )}
          </View>
    ...
    return (
      <View style={{ flex: 1, padding: 20 }}>
        ...
        <View style={{ backgroundColor: C.card, borderRadius: 20, padding: 22, borderWidth: 1, borderColor: C.border, elevation: 3 }}>
          ...
          {q.type === 'date' && (
            <View>
              {answers.incidentDate ? (
                ...
                <TouchableOpacity
                  style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: C.input, borderRadius: 12, padding: 14, borderWidth: 1.5, borderColor: C.border, gap: 10 }}
            ...
            <View style={{ gap: 10 }}>
              {q.options.map((opt) => {
                ...
                return (
                  <View key={opt}>
                    <TouchableOpacity
                      style={{ flexDirection: 'row', alignItems: 'center', padding: 14, borderRadius: 12, borderWidth: 1.5, borderColor: selected ? C.teal : C.border, backgroundColor: selected ? C.tealLight : C.input, gap: 12 }}
                    ...
                    {opt === 'Others' && selected && (
                      <TextInput
                        style={{ marginTop: 8, backgroundColor: C.input, borderWidth: 1.5, borderColor: C.teal, borderRadius: 10, padding: 12, color: C.text, fontSize: 13 }}
          ...
          {q.type === 'text' && (
            <TextInput
              style={{ backgroundColor: C.input, borderWidth: 1.5, borderColor: answers[q.key] ? C.teal : C.border, borderRadius: 12, padding: 14, color: C.text, fontSize: 14, minHeight: 80, textAlignVertical: 'top' }}
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 24, gap: 12 }}>
            <TouchableOpacity
              style={{ flex: 1, paddingVertical: 13, borderRadius: 12, borderWidth: 1.5, borderColor: C.border, alignItems: 'center' }}
              onPress={handleBack}
              activeOpacity={0.8}
            >
            ...
            <TouchableOpacity
              style={{ flex: 2, paddingVertical: 13, borderRadius: 12, backgroundColor: isAnswered() ? C.teal : C.border, alignItems: 'center', elevation: isAnswered() ? 2 : 0 }}
              onPress={handleNext}
              activeOpacity={0.85}
            >
              <Text style={{ color: isAnswered() ? '#fff' : C.sub, fontWeight: '800', fontSize: 14 }}>
                {currentQ === QUESTIONS.length - 1 ? 'Next: Personal Info →' : 'Next →'}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
              ...
              return (
                <View key={prevQ.id} style={{ flexDirection: 'row', marginBottom: 4, gap: 6 }}>
                  ...
                  <Text style={{ color: C.sub, fontSize: 11, flex: 1 }} numberOfLines={1}>
                    {pq.label.replace(/Q\d\. /, '')}: <Text style={{ color: C.text, fontWeight: '600' }}>{ans}</Text>
                  </Text>
                </View>
              );
            })}
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
      ...
      >
        <View style={{ backgroundColor: C.card, borderRadius: 20, padding: 22, borderWidth: 1, borderColor: C.border, elevation: 3 }}>
          ...
          <Text style={{ color: C.sub, fontSize: 13, marginBottom: 20, lineHeight: 18 }}>
            Please fill in all fields accurately. This information will be used for verification at the front desk.
          </Text>
          ...
          <View style={{ marginBottom: 16 }}>
            ...
            <View style={{ backgroundColor: form.age ? C.tealLight : C.input, borderWidth: 1.5, borderColor: form.age ? C.teal : C.border, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, flexDirection: 'row', alignItems: 'center' }}>
              <Text style={{ flex: 1, color: form.age ? C.teal : C.sub, fontSize: 14, fontWeight: form.age ? '700' : '400' }}>
                {form.age ? `${form.age} years old` : 'Auto-filled after selecting birthdate'}
              </Text>
            ...
            {form.address ? (
              <View style={{ backgroundColor: C.tealLight, borderRadius: 12, padding: 14, borderWidth: 1.5, borderColor: C.teal }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
                  <TouchableOpacity
                    style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: C.red, backgroundColor: dark ? '#2d1a1a' : '#ffebee' }}
                    onPress={clearAddress}
                  >
                    <Text style={{ color: C.red, fontSize: 11, fontWeight: '700' }}>🗑️ Delete</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, backgroundColor: C.teal }}
                    onPress={openAddrPicker}
                  >
          ...
          <View style={{ flexDirection: 'row', gap: 12, marginTop: 8 }}>
            <TouchableOpacity
              style={{ flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 1.5, borderColor: C.border, alignItems: 'center' }}
              onPress={() => {
                animate();
                if (editing) { setEditing(false); setPhase('view'); }
            ...
            <TouchableOpacity
              style={{ flex: 2, paddingVertical: 14, borderRadius: 12, backgroundColor: C.teal, alignItems: 'center', elevation: 3 }}
              onPress={editing ? handleSaveEdit : handleSave}
              activeOpacity={0.85}
            >
  ...
  const renderView = () => {
    ...
    if (!entry) {
      // Guard: if somehow no entry, go back to list
      setPhase('list');
      return null;
    }
    ...
    return (
      <View style={{ padding: 20 }}>
        ...
        <TouchableOpacity
          ...
          onPress={() => { animate(); setPhase('list'); setSelectedIndex(null); }}
          activeOpacity={0.8}
        >
        ...
        <View style={{ backgroundColor: '#ffffff', borderRadius: 4, padding: 24, borderWidth: 1, borderColor: '#ddd', elevation: 6, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 12, marginBottom: 16 }}>
          ...
          <Text style={{ fontSize: 9, color: '#bbb', textAlign: 'center', marginTop: 16 }}>
            This form is for ABTC-CHO use only. Please present this at the front desk.
          </Text>
        </View>
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginBottom: 8 }}>
          <TouchableOpacity
            style={{ flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 1.5, borderColor: C.red, alignItems: 'center', backgroundColor: dark ? '#2d1a1a' : '#ffebee' }}
          ...
          <TouchableOpacity
            style={{ flex: 2, paddingVertical: 14, borderRadius: 12, backgroundColor: C.teal, alignItems: 'center', elevation: 2 }}
            onPress={() => handleEdit(selectedIndex)}
            activeOpacity={0.85}
          >
  ...
  const renderDatePicker = () => {
    const daysInMonth = getDaysInMonth(dpYear, dpMonth);
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: C.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, maxHeight: '80%' }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <TouchableOpacity onPress={() => { if (dpMonth === 0) { setDpMonth(11); setDpYear(y => y - 1); } else setDpMonth(m => m - 1); }}>
                <Text style={{ color: C.teal, fontSize: 14, fontWeight: '700', paddingHorizontal: 8 }}>← Prev</Text>
              </TouchableOpacity>
              <Text style={{ color: C.text, fontSize: 15, fontWeight: '700' }}>{MONTHS[dpMonth]} {dpYear}</Text>
              <TouchableOpacity onPress={() => { if (dpMonth === 11) { setDpMonth(0); setDpYear(y => y + 1); } else setDpMonth(m => m + 1); }}>
                <Text style={{ color: C.teal, fontSize: 14, fontWeight: '700', paddingHorizontal: 8 }}>Next →</Text>
              </TouchableOpacity>
            </View>

            <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 16, marginBottom: 14 }}>
              <TouchableOpacity onPress={() => setDpYear(y => y - 1)}>
                <Text style={{ color: C.sub, fontSize: 12 }}>◀ {dpYear - 1}</Text>
              </TouchableOpacity>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 16, justifyContent: 'center' }}>
              {days.map((d) => {
                const selected = dpDay === d;
                return (
                  <TouchableOpacity
                    key={d}
                    style={{ width: 40, height: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center', backgroundColor: selected ? C.teal : C.input, borderWidth: 1, borderColor: selected ? C.teal : C.border }}
                    onPress={() => setDpDay(d)}
                  >
                    <Text style={{ color: selected ? '#fff' : C.text, fontWeight: selected ? '800' : '500', fontSize: 13 }}>{d}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
            ...
            <View style={{ flexDirection: 'row', gap: 12 }}>
              <TouchableOpacity
                style={{ flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 1.5, borderColor: C.border, alignItems: 'center' }}
                onPress={() => { setShowDatePicker(false); setDpDay(null); }}
              >
                <Text style={{ color: C.sub, fontWeight: '700' }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={{ flex: 2, paddingVertical: 14, borderRadius: 12, backgroundColor: dpDay ? C.teal : C.border, alignItems: 'center' }}
                onPress={confirmDate}
                disabled={!dpDay}
              >
                <Text style={{ color: dpDay ? '#fff' : C.sub, fontWeight: '800' }}>✓ Confirm Date</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
  ...
  const renderBdayPicker = () => {
    const daysInMonth = getDaysInMonth(bdYear, bdMonth);
    const days = Array.from({ length: daysInMonth }, (_, i) => i + 1);
      ...
      <Modal visible={showBdayPicker} transparent animationType="slide">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: C.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, maxHeight: '80%' }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <TouchableOpacity onPress={() => { if (bdMonth === 0) { setBdMonth(11); setBdYear(y => y - 1); } else setBdMonth(m => m - 1); }}>
                <Text style={{ color: C.teal, fontSize: 14, fontWeight: '700', paddingHorizontal: 8 }}>← Prev</Text>
              </TouchableOpacity>
              <Text style={{ color: C.text, fontSize: 15, fontWeight: '700' }}>{MONTHS[bdMonth]} {bdYear}</Text>
              <TouchableOpacity onPress={() => { if (bdMonth === 11) { setBdMonth(0); setBdYear(y => y + 1); } else setBdMonth(m => m + 1); }}>
                <Text style={{ color: C.teal, fontSize: 14, fontWeight: '700', paddingHorizontal: 8 }}>Next →</Text>
              </TouchableOpacity>
            </View>

            <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 16, marginBottom: 14 }}>
              <TouchableOpacity onPress={() => setBdYear(y => y - 1)}>
                <Text style={{ color: C.sub, fontSize: 12 }}>◀ {bdYear - 1}</Text>
              </TouchableOpacity>
            ...
            <View style={{ flexDirection: 'row', gap: 12 }}>
              <TouchableOpacity
                style={{ flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 1.5, borderColor: C.border, alignItems: 'center' }}
                onPress={() => { setShowBdayPicker(false); setBdDay(null); }}
              >
                <Text style={{ color: C.sub, fontWeight: '700' }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={{ flex: 2, paddingVertical: 14, borderRadius: 12, backgroundColor: bdDay ? C.teal : C.border, alignItems: 'center' }}
                onPress={confirmBirthdate}
                disabled={!bdDay}
              >
                <Text style={{ color: bdDay ? '#fff' : C.sub, fontWeight: '800' }}>✓ Confirm Birthdate</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
  ...
  const renderAddrPicker = () => {
    const DropdownSection = ({ title, icon, items, selected, onSelect, loading, isOpen, onToggle, disabled }) => (
      <View style={{ marginBottom: 12 }}>
        <TouchableOpacity
          style={{ flexDirection: 'row', alignItems: 'center', padding: 13, borderRadius: 12, borderWidth: 1.5, borderColor: selected ? C.teal : (disabled ? C.border + '60' : C.border), backgroundColor: selected ? C.tealLight : (disabled ? C.input + '80' : C.input), gap: 10 }}
            ...
            <ScrollView nestedScrollEnabled showsVerticalScrollIndicator>
              {items.map((item) => {
                ...
                return (
                  <TouchableOpacity
                    key={item.code}
                    style={{ padding: 13, borderBottomWidth: 1, borderBottomColor: C.border, backgroundColor: isSelected ? C.tealLight : C.card, flexDirection: 'row', alignItems: 'center', gap: 8 }}
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: C.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, maxHeight: '90%' }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} nestedScrollEnabled keyboardShouldPersistTaps="handled">
              ...
              <DropdownSection
                title="Region" icon="🗺️"
                items={addrRegions} selected={addrRegion} loading={addrLoading}
                isOpen={addrOpen === 'region'} onToggle={() => setAddrOpen(addrOpen === 'region' ? '' : 'region')}
                onSelect={(item) => { setAddrRegion(item); fetchProvinces(item.code); }}
                disabled={false}
              />
              <DropdownSection
                title="Province" icon="🏛️"
                items={addrProvinces} selected={addrProvince} loading={addrLoading}
                isOpen={addrOpen === 'province'} onToggle={() => setAddrOpen(addrOpen === 'province' ? '' : 'province')}
                onSelect={(item) => { setAddrProvince(item); fetchCities(item.code); }}
                disabled={!addrRegion}
              />
              <DropdownSection
                title="City / Municipality" icon="🏙️"
                items={addrCities} selected={addrCity} loading={addrLoading}
                isOpen={addrOpen === 'city'} onToggle={() => setAddrOpen(addrOpen === 'city' ? '' : 'city')}
                onSelect={(item) => { setAddrCity(item); fetchBarangays(item.code); }}
                disabled={!addrProvince}
              />
              <DropdownSection
                title="Barangay" icon="🏘️"
                items={addrBarangays} selected={addrBarangay} loading={addrLoading}
                isOpen={addrOpen === 'barangay'} onToggle={() => setAddrOpen(addrOpen === 'barangay' ? '' : 'barangay')}
                onSelect={(item) => { setAddrBarangay(item); setAddrOpen(''); }}
                disabled={!addrCity}
              />
              ...
              {addrBarangay && (
                <View style={{ backgroundColor: dark ? '#1e2d3d' : '#e3f2fd', borderRadius: 12, padding: 12, borderWidth: 1, borderColor: '#2196f3', marginBottom: 12 }}>
                  ...
                  <Text style={{ color: C.text, fontSize: 13, lineHeight: 20 }}>
                    {addrBarangay.name}, {addrCity?.name}, {addrProvince?.name}, {addrRegion?.name}, Philippines
                  </Text>
                </View>
              )}
              ...
              <View style={{ flexDirection: 'row', gap: 12, paddingBottom: 8 }}>
                <TouchableOpacity
                  style={{ flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 1.5, borderColor: C.border, alignItems: 'center' }}
                  onPress={() => setShowAddrPicker(false)}
                >
                  <Text style={{ color: C.sub, fontWeight: '700' }}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={{ flex: 2, paddingVertical: 14, borderRadius: 12, backgroundColor: addrBarangay ? C.teal : C.border, alignItems: 'center', elevation: addrBarangay ? 2 : 0 }}
                  onPress={saveAddress}
                  disabled={!addrBarangay}
                >
                  <Text style={{ color: addrBarangay ? '#fff' : C.sub, fontWeight: '800' }}>💾 Save Address</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
  ...
  // ── MAIN RENDER ──────────────────────────────────────────────────────────────
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: darkMode ? '#1a2e2c' : '#2BAF9E' }} edges={['top', 'left', 'right']}>
      <StatusBar translucent={false} backgroundColor={dark ? '#1a2e2c' : '#2BAF9E'} barStyle="light-content" />
      <SharedHeader title="📋 Patient Registration" subtitle="ABTC-CHO First Dose Registration" />
